...
public:
    ...
    virtual void acceptChildren(ASTNodeVisitor &visitor) {
        for (auto child: m_children) child->accept(visitor);
    }

    virtual void acceptTopDown(ASTNodeVisitor &visitor) {
        this->accept(visitor);
        for (auto child: m_children) child->acceptTopDown(visitor);
    }
    virtual void acceptBottomUp(ASTNodeVisitor &visitor) {
        for (auto child: m_children) child->acceptBottomUp(visitor);
        this->accept(visitor);
    }
    ...
    virtual ASTNode& getChild(int idx) {
        if (idx < 0 || idx >= m_children.size()) throw runtime_error("invalid child index");
        if (m_children[idx] == nullptr) throw runtime_error("child is NULL");
